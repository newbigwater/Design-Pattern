...
namespace _01.Creational_Pattern.FactoryMethod.Version_02
{
    public abstract class FactoryMethod_CarFactory
    {
        ...
        private readonly Dictionary<string, FactoryMethod_Car> _carMap = new Dictionary<string, FactoryMethod_Car>();
        ...
        public string ReservationCar(string carType, string reservationName)
        {
            if (string.IsNullOrWhiteSpace(reservationName))
                throw new ArgumentException("예약자 이름은 비워둘 수 없습니다.", nameof(reservationName));
        ...
        public FactoryMethod_Car ReturnCar(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
                throw new ArgumentException("조회할 예약자 이름은 비워둘 수 없습니다.", nameof(name));

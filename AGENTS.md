...
10. 패턴 문서(`doc/`) 맨 하단에는 `C# 문법 설명` 섹션을 추가하고, 해당 작업에서 실제 사용된 C# 키워드/문법을 아래 형식으로 작성한다: `대상 코드` -> `선언 의미` -> `실행 시 동작` -> `주의점/오해하기 쉬운 차이`.
11. `doc/00. CSharp 문법 스킬 맵.md`를 C# 문법 메모리 기준 문서로 사용하고, 패턴/버전 작업으로 새 문법 포인트가 확인되면 같은 작업 단위에서 반드시 갱신한다.
12. 패턴 문서 작성/수정 작업 시, 관련 코드와 직접 연결되는 C# 문법 항목을 가능한 범위에서 최대한 `C# 문법 설명` 섹션에 추가 반영한다.
